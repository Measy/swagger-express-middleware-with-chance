      ...
      switch (res.swagger.schema.type) {
        ...
        case 'array':
          ...
          if (res.swagger.schema.example) {
            ...
            try {
                routerData = fs.readFileSync(path.join(process.cwd(),`data/${global.caseName}/index.json`), { encoding: 'utf-8' });
                routerData = JSON.parse(routerData);
            } catch (error) {
                sendObject(req, res, next, parser.parse(res.swagger.schema));
            }
            if (routerData[req.method.toLowerCase()][req.swagger.pathName]) {
                sendObject(req, res, next, routerData[req.method.toLowerCase()][req.swagger.pathName]);
            } else {
                sendObject(req, res, next, parser.parse(res.swagger.schema));
            }
          }
...
function sendFile(req, res, next, file) {
  if (req.query.initDataSource) return sendObject(req, res, next, {"file": false});
